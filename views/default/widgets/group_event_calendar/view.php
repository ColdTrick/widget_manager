<?php 
	...
	if($events = event_calendar_get_events_between($start_date, $end_date, false, $event_count, 0, $group->getGUID())) {
		...
		echo elgg_view("output/url", array("href" => $vars["url"] . "pg/event_calendar/group/" . $group->getGUID(), "text" => elgg_echo('event_calendar:site_wide_link')));
		echo "</div>";
	} else {
		echo elgg_view("page_elements/contentwrapper", array("body" => elgg_echo("event_calendar:no_events_found")));
	}
