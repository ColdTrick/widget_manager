<?php 
	...
	echo elgg_view("input/text", array("internalname" => "params[rssfeed]", "value" => $entity->rssfeed));
	echo "</div>";
	
	echo "<div>";
	echo elgg_echo("widgets:rss:settings:rss_count") . " "; 
	echo elgg_view("input/pulldown", array("internalname" => "params[rss_count]", "options" => range(1,10), "value" => $rss_count)); 
	echo "</div>";
	
	echo "<div>";
	echo elgg_echo("widgets:rss:settings:show_feed_title") . " "; 
	echo elgg_view("input/pulldown", array("internalname" => "params[show_feed_title]", "options_values" => $yesno_options, "value" => $entity->show_feed_title));
	echo "</div>";
